...
const cloesSearchTabletBtn = $.querySelector(".cloes-search-table");
...
function stopedLoader (){
  loader.classList.add("disabel")
}

function startLoader (){
  loader.classList.remove("disabel")
}
...
function selectMethodSearch() {
  ...
  searchUserMetodItems.forEach( function (metodItem) {
    ...
    metodItem.addEventListener("click", e => {
      searchMethodValue = e.target.dataset.searchmethod
      userSearchDropdownlist.classList.remove('show')
    })
  })
}
...
usersSearchInput.addEventListener('input', function (event) {
  ...
  const obj = {
    method: searchMethodValue ? searchMethodValue : "username",
    text: inputValue,
  };    
...
async function search(obj) {
  try{
    const res = await fetch("https://filimo-copy.iran.liara.run/user/search?" +
...
function addTargetUserToTabel(user) {
  usersTableHead.innerHTML = ""
  usersTableBody.innerHTML = ""

  cloesSearchTabletBtn.style.display = 'block';
...
function cloesSearchTableHandeler () {
  usersSearchInput.value = ""
  cloesSearchTabletBtn.style.display = 'none';
  getUsers (token)
}
